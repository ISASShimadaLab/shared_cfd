...
subroutine Fex(neq_outer, tt, n, dndt, rpar, ipar)!{{{
   ...
   !!! FOR hp problem rho*D(N) -> D(rho*N)!!!
   tmp=tmp/sn+dndt(ns+1)/T
   do j=1,ns
      dndt(j)=-tmp*n(j)+dndt(j)
   end do
end subroutine Fex!}}}
subroutine Fex_var(n, dndtp, dndtm)!dummy{{{
   !variables
   use const_chem
   implicit none
   double precision n(ns+1)
   double precision dndtp(ns+1)
   double precision dndtm(ns+1)
...
subroutine Jex(neq_outer, tt, n, ML, MU, dndn, nrpd, rpar, ipar)!{{{
   ...
   do j=1,ns+1
      ...
      tmp=tmp/sn+dndn(ns+1,j)/T
      do i=1,ns
         dndn(i,j)=-tmp*n(i)+dndn(i,j)
      end do
   end do
end subroutine Jex!}}}
