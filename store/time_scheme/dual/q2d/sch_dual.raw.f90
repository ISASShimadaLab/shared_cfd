   ...
   do plane=nps,npe
      ...
      do j=nys(plane),nye(plane)
         do i=nxs(plane),nxe(plane)
            ...
            dtau = cfl_tau*Vol(i,j,plane)/max(uds,w(indxMu,i,j,plane)/rho)

            !set preconditioner
            !set scalars
            alpha(i,j,plane)= 1d0/(Vol(i,j,plane)/dtau + dsci(i,j,plane)*nuA + dscj(i,j,plane)*nuB)
            beta      = 3d0*Vol(i,j,plane)/(2d0*DT_LOCAL_GLOBAL) *alpha(i,j,plane)
            
            pre_sca(1,i,j,plane) = 1d0+beta                                       !for S and S^(-1)
            pre_sca(2,i,j,plane) = -1d0/(       1d0/phi(i,j,plane)+c**2)                !for Gamma^(-1)
            pre_sca(3,i,j,plane) = -1d0/(       1d0/phi(i,j,plane)+c**2)/(1d0/beta+1d0) !for S
            pre_sca(4,i,j,plane) = beta/((1d0+beta)/phi(i,j,plane)+c**2)                !for S^(-1)

            !set vectors
            do k=1,nY
               pre1(k,i,j,plane) = w(4+k,i,j,plane)
            end do
            pre1(nY+1,i,j,plane) = u
            pre1(nY+2,i,j,plane) = v
            pre1(nY+3,i,j,plane) = H
