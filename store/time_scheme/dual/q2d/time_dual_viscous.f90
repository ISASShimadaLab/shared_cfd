subroutine calc_next_step_implicit(step_internal)
   ...
   do plane = nps,npe
      ...
      do j=nys(plane),nye(plane)
         do i=nxs(plane),nxe(plane)
            ...
            Dq(:,i,j,plane)= Dq_small-Vol(i,j,plane)*dqdt(:,i,j,plane)

            Dq(:,i,j,plane)=Dq(:,i,j,plane)+pre_sca(2,i,j,plane)*dot_product(dpdq(:,i,j,plane),Dq(:,i,j,plane))*pre1(:,i,j,plane)
         end do
      end do
      !$omp end parallel do
      ...
      do sm=nxs(plane)+nys(plane),nxe(plane)+nye(plane)
         ...
         do i=max(nxs(plane),sm-nye(plane)),min(sm-nys(plane),nxe(plane))
            ...
            Dq(:,i,j,plane)=Dq(:,i,j,plane)+pre_sca(4,i,j,plane) *dot_product(dpdq(:,i,j,plane),Dq(:,i,j,plane)) *pre1(:,i,j,plane)
            Dq(:,i,j,plane)=1d0/pre_sca(1,i,j,plane)*Dq(:,i,j,plane)
         end do
         !$omp end parallel do
      end do
      ...
      do j=nys(plane),nye(plane)
         do i=nxs(plane),nxe(plane)
            Dq(:,i,j,plane)=Dq(:,i,j,plane)+pre_sca(3,i,j,plane) *dot_product(dpdq(:,i,j,plane),Dq(:,i,j,plane)) *pre1(:,i,j,plane)
            Dq(:,i,j,plane)=pre_sca(1,i,j,plane)*Dq(:,i,j,plane)
         end do
      end do
      !$omp end parallel do
   ...
   if(myid .eq. 0) then
      ...
      if(ILwrite) write(66,'(i3,100e12.3)') step_internal,omega,res/res1,dt_grbl
   end if
