      ...
      case(  1)
         if(gx(plane) .eq. 1) then
            ...
            do j=max(    1,nys(plane)),min(nye(plane),   99)
               w(     1,0,j,plane)=w(4,1,j,plane)*MWo/(R_uni*To)
               w(     2,0,j,plane)=0.1d0
               w(     3,0,j,plane)=0d0
               w(     4,0,j,plane)=w(4,1,j,plane)

               w(4+1:4+nY,0,j,plane)=vwo(1:nY)

               w(indxg, 0,j,plane)=kappao
               w(indxht,0,j,plane)=Eo+R_uni/MWo*To +0.5d0*(w(2,0,j,plane)**2+w(3,0,j,plane)**2)
               w(indxR, 0,j,plane)=R_uni/MWo
               w(indxMu,0,j,plane)=muo
               vhi(:,   0,j,plane)=vhio
