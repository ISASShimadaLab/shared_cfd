   ...
   do plane=nps,npe
      do i=nxs(plane),nxe(plane)
         do j=nys(plane),nye(plane)
            ...
            q(2,    i,j,plane) = 0d0
            q(3,    i,j,plane) = 0d0
            q(4,    i,j,plane) = 1d5/(kappa_gas-1d0)
            w(4,    i,j,plane) = 1d5
   ...
   do plane=nps,npe
      ...
      case(  1)
         if(gx(plane) .eq. 1) then
            ...
            do j=max(    1,nys(plane)),min(nye(plane),  100)
               ...
               w(indxht,0,j,plane) = kappa_gas/(kappa_gas-1d0)*R_gas*300d0&
                                   + 0.5d0*(w(2,0,j,plane)**2+w(3,0,j,plane)**2)
               w(indxR ,0,j,plane) = R_gas
               w(indxMu,0,j,plane) = rho*nu_gas
               vhi(1, 0,j,plane) = kappa_gas/(kappa_gas-1d0)*R_gas*300d0
         ...
         if(gx(plane) .eq. ngx(plane)) then
            !i=ni+1/2
            do j=max(    1,nys(plane)),min(nye(plane),   50)
      ...
      case(  2)
         if(gx(plane) .eq. ngx(plane)) then
            ...
            do j=max(    1,nys(plane)),min(nye(plane),   50)
               w(:,  ni(plane)+1,j,plane) = w(:,  ni(plane),j,plane) 
               w(4,  ni(plane)+1,j,plane) = w(4,  ni(plane),j,plane) 
               vhi(:,ni(plane)+1,j,plane) = vhi(:,ni(plane),j,plane) 
   ...
   do plane=nps,npe
      ...
      case(  1)
         if(gy(plane) .eq. 1) then
            !j=1/2
            do i=max(    1,nxs(plane)),min(nxe(plane),   50)
