   ...
   do plane=nps,npe
      do i=nxs(plane),nxe(plane)
         do j=nys(plane),nye(plane)
            ...
            q(2,    i,j,plane)=rho*u
            q(3,    i,j,plane)=rho*v
            q(4,    i,j,plane)=rho*(1d0/(kappa_gas-1d0)*R_gas*T+0.5d0*(u**2+v**2))
            w(4,    i,j,plane)=1.54148755777d5
      ...
      case(  1)
         ...
         if(gx(plane) .eq. ngx(plane)) then
            ...
            do j=max(    1,nys(plane)),min(nye(plane),  100)
               ...
               w(4,  ni(plane)+1,j,plane) = 1.54148755777d5
   ...
   do plane=nps,npe
      ...
      case(  1)
         if(gy(plane) .eq. 1) then
            !j=1/2 lower wall
            !slip wall
            do i=nxs(plane)-1,min(nxe(plane)+1,200)
               w(:,  i, 0,plane) =  w(:,  i,1,plane)
               dp = vnj(1,i,0,plane)*w(2,i,1,plane)+vnj(2,i,0,plane)*w(3,i,1,plane)
               w(2,  i, 0,plane) =  w(2,  i,1,plane)-2d0*dp*vnj(1,i,0,plane)
               w(3,  i, 0,plane) =  w(3,  i,1,plane)-2d0*dp*vnj(2,i,0,plane)
            ...
            !mirror wall
            do i=max(nxs(plane)-1,201),nxe(plane)+1
               w(:,  i, 0,plane) =  w(:,  i,1,plane)
         ...
         if(gy(plane) .eq.  ngy(plane)) then
            !j=nj+1/2
            do i=max(    1,nxs(plane)),min(nxe(plane),  250)
               T = 225.31883809d0
               w(     1,i,nj(plane)+1,plane)=w(4,i,nj(plane),plane)/(R_gas*T)
               w(     2,i,nj(plane)+1,plane)=121.347809114d0
               w(     3,i,nj(plane)+1,plane)=0d0
               w(     4,i,nj(plane)+1,plane)=w(4,i,nj(plane),plane)
      
               w(4+1:4+nY,i,nj(plane)+1,plane)=1d0
      
               w(indxg, i,nj(plane)+1,plane)=kappa_gas
               w(indxht,i,nj(plane)+1,plane)=kappa_gas/(kappa_gas-1d0)*R_gas*T+0.5d0*(w(2,i,nj(plane)+1,plane)**2+w(3,i,nj(plane)+1,plane)**2)
               w(indxR, i,nj(plane)+1,plane)=R_gas
               w(indxMu,i,nj(plane)+1,plane)=nu_gas*w(1,i,nj(plane)+1,plane)
