   ...
   do plane=nps,npe
      ...
      case(  1)
         if(gx(plane) .eq. 1) then
            !i=1/2
            do j=max(    1,nys(plane)),min(nye(plane),   35)
               w(:,   0,j,plane) = w(:,   1,j,plane)
               w(2,   0,j,plane) =-w(2,   1,j,plane)
      ...
      case(  2)
         if(gx(plane) .eq. 1) then
            !i=1/2
            do j=max(    1,nys(plane)),min(nye(plane),   21)
               if (j<=12) then
                  select case(j)
                  case(1)
                     rho=3.016d0;u=1002.d0;v=3.349d0;P=260791.d0
                  case(2)
                     rho=3.019d0;u=1002.d0;v=10.04d0;P=261074.d0
                  case(3)
                     rho=3.023d0;u=1001.d0;v=16.71d0;P=261641.d0
                  case(4)
                     rho=3.031d0;u=1001.d0;v=24.20d0;P=262570.d0
                  case(5)
                     rho=3.041d0;u=1000.d0;v=31.69d0;P=263731.d0
                  case(6)
                     rho=3.050d0;u=999.8d0;v=38.41d0;P=264913.d0
                  case(7)
                     rho=3.060d0;u=999.1d0;v=45.24d0;P=266125.d0
                  case(8)
                     rho=3.069d0;u=998.4d0;v=52.27d0;P=267269.d0
                  case(9)
                     rho=3.076d0;u=997.6d0;v=59.57d0;P=268178.d0
                  case(10)
                     rho=3.080d0;u=996.8d0;v=67.19d0;P=268728.d0
                  case(11)
                     rho=3.078d0;u=996.0d0;v=75.08d0;P=268790.d0
                  case(12)
                     rho=2.831d0;u=994.7d0;v=88.64d0;P=247739.d0
                  end select
                  T=P/(rho*R_gas)
                  w(1,     -1:0,j,plane) = rho
                  w(2,     -1:0,j,plane) = u
         ...
         if(gx(plane) .eq. ngx(plane)) then
            !i=ni+1/2
            do j=max(    1,nys(plane)),min(nye(plane),   56)
               w(:,  ni(plane)+1,j,plane) = w(:,  ni(plane)  ,j,plane)
               w(2,  ni(plane)+1,j,plane) =-w(2,  ni(plane)  ,j,plane)
