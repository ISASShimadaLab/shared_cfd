   ...
   do plane=nps,npe
      do i=nxs(plane),min(nxe(plane),54)
         do j=nys(plane),nye(plane)
            q(:,    i,j,plane) =qo
            q(nY+1, i,j,plane) =rhoo*100d0
            q(nY+3, i,j,plane) =q(nY+3, i,j,plane)+0.5d0*rhoo*100d0**2
            w(4,    i,j,plane) =po
      ...
      do i=max(nxs(plane),55),nxe(plane)
         do j=nys(plane),nye(plane)
            q(:,    i,j,plane) =qf
            q(nY+1, i,j,plane) =rhof*100d0
            q(nY+3, i,j,plane) =q(nY+3, i,j,plane)+0.5d0*rhof*100d0**2
            w(4,    i,j,plane) =pf
   ...
   do plane=nps,npe
      ...
      case(  1)
         if(gx(plane) .eq. 1) then
            !i=1/2
            do j=max(    1,nys(plane)),min(nye(plane),   99)
               w(:,     0,j,plane) = wo
               w(2,     0,j,plane) = 100d0
               w(indxht,0,j,plane) = wo(indxht)+0.5d0*100d0**2
               vhi(:,   0,j,plane) = vhio
               Yv(:,    0,j,plane) = Yvo
   ...
   do plane=nps,npe
      ...
      case(  1)
         if(gy(plane) .eq. 1) then
            !j=1/2
            do i=max(    1,nxs(plane)),min(nxe(plane),  54)
            ...
            do i=max(   55,nxs(plane)),min(nxe(plane),  223)
               w(:,     i, 0,plane) = wf
               w(2,     i, 0,plane) =-w(2,i,1,plane)
               w(3,     i, 0,plane) = 10d0
               w(indxht,i, 0,plane) = wf(indxht)+0.5d0*(10d0**2+w(2,i,1,plane)**2)
               vhi(:,   i, 0,plane) = vhif
               Yv(:,    i, 0,plane) = Yvf
