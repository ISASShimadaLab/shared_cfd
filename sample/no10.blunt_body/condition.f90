subroutine set_IC
   ...
   T=300d0
   u=2.49d0*sqrt(kappa_gas*R_gas*T)
   rho=1d5/(R_gas*T)
   ...
   do plane=nps,npe
      do i=nxs(plane),nxe(plane)
         do j=nys(plane),nye(plane)
            ...
            q(2,    i,j,plane) = rho*u
            q(3,    i,j,plane) = 0d0
            q(4,    i,j,plane) = 1d5/(kappa_gas-1d0) + 0.5d0*rho*u**2
            w(4,    i,j,plane) = 1d5
   ...
   do plane=nps,npe
      ...
      case(  1)
         if(gy(plane) .eq. 1) then
            !j=1/2
            do i=max(    1,nxs(plane)),min(nxe(plane),  199)
         ...
         if(gy(plane) .eq.  ngy(plane)) then
            ...
            do i=max(    1,nxs(plane)),min(nxe(plane),  199)
               ...
               w(indxMu,i,nj(plane)+1,plane) = nu_gas*rho
               vhi(1,i,nj(plane)+1,plane) = kappa_gas/(kappa_gas-1d0)*R_gas*T
