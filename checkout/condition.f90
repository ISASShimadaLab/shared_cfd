      ...
      case(  1)
         if(gx(plane) .eq. 1) then
            ...
            do j=max(    1,nys(plane)),min(nye(plane),   99)
               T   = 300d0
               rho = w(4,1,j,plane)/(R_gas*T)
               w(     1,0,j,plane)= rho
               w(     2,0,j,plane)= 0.1d0
               w(     3,0,j,plane)= 0d0
               w(     4,0,j,plane)= w(4,1,j,plane)

               w(4+1:4+nY,0,j,plane)= 1d0

               w(indxg, 0,j,plane)=kappa_gas
               w(indxht,0,j,plane)=kappa_gas/(kappa_gas-1d0)*R_gas*T +0.5d0*(w(2,0,j,plane)**2+w(3,0,j,plane)**2)
               w(indxR, 0,j,plane)=R_gas
               w(indxMu,0,j,plane)=nu_gas*rho
